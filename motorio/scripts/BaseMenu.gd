...
func _draw() -> void:
	if main_controller == null or not main_controller.base_menu_open:
		return
	var panel: Rect2 = Rect2(size / 2.0 - Vector2(160, 112), Vector2(320, 224))
	...
	draw_string(font, panel.position + Vector2(22, 154), status, HORIZONTAL_ALIGNMENT_LEFT, -1, 13, Color("8dd9cb"))
	draw_string(font, panel.position + Vector2(22, 196), "Z CRAFT   X SELECT   RUN CLOSE", HORIZONTAL_ALIGNMENT_LEFT, -1, 12, Color("b9c5be"))
...
func _draw_recipe(font: Font, panel: Rect2, index: int, y: float, label: String) -> void:
	...
	if selected:
		draw_rect(Rect2(panel.position + Vector2(14, y - 24), Vector2(292, 31)), Color(0.16, 0.25, 0.21, 0.9))
	...
	draw_string(font, panel.position + Vector2(236, y), "3 BOX", HORIZONTAL_ALIGNMENT_LEFT, -1, 14, Color("e7aa45"))

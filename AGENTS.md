...
- 하루는 빠른 시작 완료 후 흐르는 12분이며 11분부터 고양이가 작업을 멈추고 귀가한다. 플레이어는 기지 남서 `HOME`에서 모닥불 취침하며, 체온 소진이나 12분 도달 시 고양이가 구조한다.
- 전력·식량 생산은 각각 발전기×전기 고양이, 치즈 밭×요리 고양이의 작은 쪽 개수만큼 확장한다. 추가 고양이만으로 장비 용량을 초과하지 않으며 HEAT/POWER/FOOD 기술은 반복할수록 비용과 효과가 증가한다.
